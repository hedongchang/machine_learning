...
public class OnePredict extends RecursiveTask<Pair<Float, Float>>{
	...
	public OnePredict(int lo, int hi, HashMap<Integer, HashMap<Integer, Float>> data,
	...
	protected Pair<Float, Float> compute() {
		if (hi - lo <= 1000 / 8)  {
			...
			OnePredict left = new OnePredict(lo, (hi + lo) / 2, data, userList, user, movie, userAvg);
			OnePredict right = new OnePredict((hi + lo) / 2, hi, data, userList, user, movie, userAvg);
			left.fork();
	...
	private synchronized float computeAverage(HashMap<Integer, Float> movies) {
		float ratingSum = 0;
	...
	private synchronized float findRating(int user, int movie) {
